...
export const MessageInput = React.memo(function MessageInput({
  ...
  const [roomMembers, setRoomMembers] = useState([]);  const [showMentionUI, setShowMentionUI] = useState(false);
  ...
  // 답장 이벤트 리스너 등록 (팝업 모드가 아닐 때만)
  useEffect(() => {
    let handleReplyEvent;
    if (!isPopup) {
      handleReplyEvent = (event) => {
        const { messageId, content, senderName, replyTo } = event.detail;
        setReplyTo({ messageId, content, senderName, replyTo });
        setTimeout(() => {
          if (inputRef.current) {
      ...
      window.addEventListener("replyToMessage", handleReplyEvent);
    }

    const handleDropFilesEvent = (event) => {
      const { files, type } = event.detail;
      // useEffect 의존성에서 handleFileSelect를 제거하고, 최신 selectedFiles를 위해 setTimeout으로 비동기 처리
      setTimeout(() => handleFileSelect(files, type), 0);
    };
    window.addEventListener("dropFiles", handleDropFilesEvent);

    return () => {
      if (!isPopup && handleReplyEvent) {
        window.removeEventListener("replyToMessage", handleReplyEvent);
      }
      window.removeEventListener("dropFiles", handleDropFilesEvent);
    };
  }, [isPopup]);

  // 입력창 포커스 함수 선언을 최상단에 위치
  ...
  // 파일 선택 핸들러를 focusInput 아래에 위치
  const handleFileSelect = useCallback(
    (files, fileType) => {
      // 파일 타입별 제한 확인
      const existingImageFiles = selectedFiles.filter(
        (f) => f.fileCategory === "image"
      );
      const existingVideoFiles = selectedFiles.filter(
        (f) => f.fileCategory === "video"
      );
      const existingDocFiles = selectedFiles.filter(
        (f) => f.fileCategory === "file"
      );

      // 제한 검사
      if (
        fileType === "image" &&
        existingImageFiles.length + files.length > 6
      ) {
        alert("이미지는 최대 6개까지만 첨부할 수 있습니다.");
        return;
      }

      if (fileType === "video" && existingVideoFiles.length >= 1) {
        alert("동영상은 1개만 첨부할 수 있습니다.");
        return;
      }

      if (fileType === "file" && existingDocFiles.length >= 1) {
        alert("파일은 1개만 첨부할 수 있습니다.");
        return;
      }

      // 동영상과 파일은 1개만 선택되도록 제한
      ...
      // 이미지는 최대 6개까지만 선택 가능
      if (fileType === "image") {
        const remainingSlots = 6 - existingImageFiles.length;
        filesToProcess = filesToProcess.slice(0, remainingSlots);
      }

      const filesWithInfo = filesToProcess.map((file) => {
  ...
  useEffect(() => {
    const handleBlur = (e) => {
      ...
      if (!isSending && activeRoom && shouldFocusRef.current) {
        setTimeout(focusInput, 50);
      }
    };
  ...
  }, [activeRoom, isSending, focusInput]);
  ...
  const filterFilesByType = useCallback(
    (acceptType) => {
      ...
      // 파일 타입별 multiple 속성 설정
      if (acceptType === "image") {
        const remainingImageSlots =
          6 - selectedFiles.filter((f) => f.fileCategory === "image").length;
        input.multiple = remainingImageSlots > 1;
      } else {
        // 비디오와 일반 파일은 단일 선택만 가능
  ...
  const processMentions = (messageText) => {
    ...
    return matches
      .map((match) => {
        ...
        const mentionedUser = activeRoom?.members.find(
  ...
  const handleSendMessage = async () => {
    ...
    if ((messageToSend === "" && !hasFiles) || isSending || disabled) return;
    ...
    try {
      ...
      console.log("답장 처리된 메시지:", messageData);

      if (filesToSend.length > 0 && activeRoom) {
        ...
        success = await sendWithFileMessage(messageToSend, fileAttachments);
      } else if (activeRoom) {
        success = await sendMessage(messageToSend, replyId, replyContent);
      }

      console.log("메시지 전송 완료:", { success });

      if (scheduleFocus) scheduleFocus();
      ...
      // 답장 상태 초기화
      if (setReplyTo) setReplyTo(null);    } catch (error) {
  ...
  const isDisabledCalc = !activeRoom || isSending || disabled;
  useEffect(() => {
    if (setIsDisabled) setIsDisabled(isDisabledCalc);
  }, [activeRoom, isSending, isDisabledCalc, disabled, setIsDisabled]);let placeholder = "";
  // placeholder 텍스트 모두 제거
    return (
    <div className="p-1 md:p-1.5 bg-white border-t border-[#E0E0E0] sticky bottom-0 w-full">      {/* 답장 UI (팝업에서도 사용 가능) */}
      {replyTo && (
        <div className="mx-2 mb-2 p-2 bg-gray-50 border-l-4 border-blue-400 rounded flex justify-between items-start">
        ...
        {selectedFiles.map((file) => (
          <div key={file.id} className="relative group">
            {file.type.startsWith("image/") ? (
              <div className="relative w-24 h-24 md:w-32 md:h-32 rounded-md overflow-hidden border border-[#E0E0E0] bg-white">
                <img
              ...
              <div className="relative w-24 h-24 md:w-32 md:h-32 rounded-md overflow-hidden border border-[#E0E0E0] bg-white flex items-center justify-center">
        ...
        <DropdownMenu>
          ...
          >
            ...
            >
              ...
              <span>이미지 첨부</span>            </DropdownMenuItem>
            <DropdownMenuItem
              className="flex items-center cursor-pointer hover:bg-[#E0E0E0]"
          ...
          <div className="flex justify-center items-center gap-3">
            ...
            <>
              ...
              >
                ...
                <EmojiPicker
                  onEmojiSelect={handleEmojiSelect}                  disabled={isDisabled}
                />
              </>
            <button
              className={`${
